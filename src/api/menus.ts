...
export const menusApi: IMenuApi =  {
    getMenus: ({page = 1}: { page: number }, {signal}: {signal: AbortSignal}) => {
        return fetch(`${BASE_URL}/maraphones?_page=${page}&_per_page=10`, {signal}).then(res => res.json()) as Promise<Response<Menu[]>>;
    },
    getMenuDays: ({maraphone_id = 1}: { maraphone_id: number }, {signal}: {signal: AbortSignal}) => {
    ...
    getMenuDay: ({maraphone_id = 1, day_id = 1}: { maraphone_id: number, day_id: number }, {signal}: {signal: AbortSignal}) => {
